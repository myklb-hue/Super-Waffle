...
a memory store, an actuator, a control structure, or a piece of your own
code. A wire is a typed connection. A graph runs once, runs live against a
stream of events, or runs on a schedule.

Single-user, local-first. The user owns the machine, the models, the
tools and the data.
...
The mockups are also on a design canvas, and the spec as a page with
figures — links are in the spec's opening paragraph.

## Status

Design. No application code yet.
